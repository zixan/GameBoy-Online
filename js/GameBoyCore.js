...
GameBoyCore.prototype.returnFromState = function (returnedFrom) {
	...
	this.channel1ConsecutiveBug = state[index++];
	...
	this.channel2ConsecutiveBug = state[index++];
	this.channel2frequency = state[index++];
	this.channel2Fault = state[index++];
	...
	this.channel3Fault = state[index++];
	this.channel3consecutive = state[index++];
	this.channel3ConsecutiveBug = state[index++];
...
GameBoyCore.prototype.initializeAudioStartState = function (resetType) {
	if (resetType) {
		...
		this.channel1ConsecutiveBug = false;
		...
		this.channel2ConsecutiveBug = false;
		this.channel2frequency = 0;
		this.channel2Fault = false;
		...
		this.channel3Fault = false;
		this.channel3consecutive = true;
		this.channel3ConsecutiveBug = false;
...
GameBoyCore.prototype.audioChannelsComputeStereo = function () {
	//Channel 1:
	if (((this.channel1consecutive && !this.channel1ConsecutiveBug) || this.channel1totalLength > 0) && this.channel1frequency <= 0x7FF && !this.channel1Fault) {
	...
	//Channel 2:
	if (((this.channel2consecutive && !this.channel2ConsecutiveBug) || this.channel2totalLength > 0) && !this.channel3Fault) {
		...
		if (this.channel2volumeEnvTimeLast > 0) {
	...
	//Channel 3:
	if (this.channel3canPlay && ((this.channel3consecutive && !this.channel3ConsecutiveBug) || this.channel3totalLength > 0) && !this.channel3Fault) {
	...
	if (this.channel4consecutive || this.channel4totalLength > 0) {
		...
		if (this.channel4volumeEnvTimeLast > 0) {
...
GameBoyCore.prototype.audioChannelsComputeMono = function () {
	//Channel 1:
	if (((this.channel1consecutive && !this.channel1ConsecutiveBug) || this.channel1totalLength > 0) && this.channel1frequency <= 0x7FF && !this.channel1Fault) {
		this.currentSampleRight = (this.channel1lastSampleLookup <= this.channel1adjustedDuty && this.rightChannel[0]) ? this.channel1currentVolume : 0;
		if (this.channel1numSweep > 0) {
			if (--this.channel1timeSweep == 0) {
				--this.channel1numSweep;
				this.channel1frequency = (this.channel1ShadowFrequency > -1) ? this.channel1ShadowFrequency : this.channel1frequency;
	...
	//Channel 2:
	if (((this.channel2consecutive && !this.channel2ConsecutiveBug) || this.channel2totalLength > 0) && !this.channel3Fault) {
		if (this.channel2lastSampleLookup <= this.channel2adjustedDuty && this.rightChannel[1]) {
			this.currentSampleRight += this.channel2currentVolume;
		}
		if (this.channel2volumeEnvTimeLast > 0) {
			if (this.channel2volumeEnvTime > 0) {
...
GameBoyCore.prototype.audioChannelsComputeStereoSafe = function () {
	...
	//Channel 2:
	if (((this.channel2consecutive && !this.channel2ConsecutiveBug) || this.channel2totalLength > 0) && !this.channel2Fault) {
		if (this.channel2lastSampleLookup <= this.channel2adjustedDuty) {
			if (this.leftChannel[1]) {
				this.currentSampleLeft += this.channel2currentVolume;
			}
...
GameBoyCore.prototype.renderMidScanLine = function () {
	if (this.actualScanLine < 144 && this.LCDTicks >= 82 && this.modeSTAT == 3 && (settings[4] == 0 || this.frameCount > 0)) {
		//TODO: Get this accurate:
		if (this.midScanlineOffset == -1) {
			this.midScanlineOffset = this.memory[0xFF43] & 0x7;
		}
		if (this.LCDTicks >= 90) {
			var pixelEnd = this.LCDTicks - 82;
...
GameBoyCore.prototype.consoleModeAdjust = function () {
	...
	if (this.OBJPalette) {
		var index = this.OBJPalette.length;
		while (index > 0) {
			this.OBJPalette[--index] |= 0x1000000;
		}
	}
	...
	if (this.gfxWindowDisplay) {									//Is the window enabled?
		...
		if (scrollYAdjusted >= 0) {
			...
			if (pixelPosition < pixelPositionEnd) {
				...
				var scrollXAdjustedAligned = Math.min(pixelPositionEnd - pixelPosition, 160 - scrollXAdjusted) >> 3;
				scrollXAdjusted += scrollXAdjustedAligned << 3;
				scrollXAdjustedAligned += tileNumber;
				while (tileNumber < scrollXAdjustedAligned) {
					chrCode = this.BGCHRBank1[++tileNumber];
...
GameBoyCore.prototype.SpriteGBLayerRender = function () {
	if (this.gfxSpriteShow) {										//Is the window enabled?
		...
		if (!this.gfxSpriteDouble) {
			var lowestSpriteAddress = this.findLowestSpriteDrawable();
			...
			var lowestSpriteAddress = this.findLowestSpriteDoubleDrawable();
			//Draw the visible sprites:
			for (var onXCoord = 1; onXCoord < 8; onXCoord++) {
				currentColumn = this.OAMAddresses[onXCoord];
				length = currentColumn.length;
...
GameBoyCore.prototype.registerWriteJumpCompile = function () {
	...
	this.memoryHighWriter[0x13] = this.memoryWriter[0xFF13] = function (parentObj, address, data) {
		if (parentObj.soundMasterEnabled) {
			parentObj.audioJIT();
			if ((parentObj.channel1consecutive || parentObj.channel1totalLength > 0) && parentObj.channel1frequency <= 0x7FF && !parentObj.channel1Fault) {
