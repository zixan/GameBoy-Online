...
function GameBoyCore(canvas, ROMImage) {
	...
	this.dutyLookup = [1, 2, 4, 6];					//Map the duty values given to ones we can work with.
...
GameBoyCore.prototype.clockAudioEnvelope = function () {
	//Channel 1:
	if (this.channel1envelopeSweepsLast > 0) {
	...
	//Channel 2:
	if (this.channel2envelopeSweepsLast > 0) {
	...
	//Channel 4:
	if (this.channel4envelopeSweepsLast > 0) {
...
GameBoyCore.prototype.registerWriteJumpCompile = function () {
	...
	this.memoryHighWriter[0x10] = this.memoryWriter[0xFF10] = function (parentObj, address, data) {
		if (parentObj.soundMasterEnabled) {
			...
			parentObj.channel1lastTimeSweep = parentObj.channel1timeSweep = (data & 0x70) >> 4;
	...
	this.memoryHighWriter[0x12] = this.memoryWriter[0xFF12] = function (parentObj, address, data) {
		if (parentObj.soundMasterEnabled) {
			...
			if (data < 0x08) {
				...
				parentObj.channel1envelopeVolume = 0;
			}
		...
		if (parentObj.soundMasterEnabled) {
			...
			if (data > 0x7F) {
				...
				if (nr12 > 0x07) {
					...
					parentObj.channel1envelopeSweepsLast = nr12 & 0x7;
	...
	this.memoryHighWriter[0x17] = this.memoryWriter[0xFF17] = function (parentObj, address, data) {
		if (parentObj.soundMasterEnabled) {
			...
			if (data < 0x08) {
				//Manual says this is a way to turn off the audio:
				parentObj.channel2envelopeVolume = 0;
			}
		...
		if (parentObj.soundMasterEnabled) {
			...
			if (data > 0x7F) {
				...
				if (nr22 > 0x7) {
					...
					parentObj.channel2envelopeSweepsLast = nr22 & 0x7;
		...
		if (parentObj.soundMasterEnabled) {
			...
			if (data > 0x7F) {
				...
				if (nr42 > 0x7) {
					...
					parentObj.channel4currentVolume = parentObj.channel4envelopeVolume << parentObj.channel4VolumeShifter;
					parentObj.channel4envelopeSweepsLast = nr42 & 0x7;
